...
function AdminDashboard({ user, db, onManageSession, onCreateSession, onManageAdmins, onLogout, onDeleteSession }: AdminDashboardProps) {
    ...
    return ( <>
        ...
        <div className="bg-white p-6 rounded-lg shadow-xl border border-slate-200 w-full"> 
            ...
            <div className="space-y-4 max-h-96 scroll-list"> {userSessions.length > 0 ? userSessions.map(session => ( <div key={session.id} className="bg-slate-50 border border-slate-200 p-4 rounded-lg"> <div className="flex justify-between items-start gap-3"> <div className="min-w-0"><h4 className="font-bold text-slate-800">{session.name}</h4><p className="text-sm text-slate-500">{session.members.length} miembros</p></div> <div className="flex items-center gap-2 shrink-0"> <button onClick={() => session.id && onManageSession(session.id)} className="bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold py-2 px-4 rounded-lg text-sm">Gestionar</button> {onDeleteSession && session.id && <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget({ id: session.id!, name: session.name }); }} className="min-w-[44px] min-h-[44px] p-2 flex items-center justify-center rounded-lg text-red-600 hover:bg-red-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 focus-visible:ring-offset-2" aria-label="Eliminar sesión" title="Eliminar"><TrashIcon /></button>} </div> </div> </div> )) : <p className="text-slate-500 text-center py-4">No has creado ninguna sesión.</p>} </div> 
        </div>
    </> );
}
...
function SessionManagement({ session: initialSession, votes: initialVotes, onAccredit, onUnaccredit, onToggleEligibility: _onToggleEligibility, onChangeElectionStatus, onAddElection, onAddMembers, onBack, onViewResults: _onViewResults, onReorderElections, isXlsxLoaded, user, onDeleteMember, onDeleteElection }: SessionManagementProps) {
    ...
    const downloadMemberList = () => { if (!isXlsxLoaded) { alert("La librería de exportación no está lista."); return; } const lib = (window as any).XLSX; if (!lib) { alert("La librería de exportación no está lista."); return; } const data = session.members.map(({ name, key, email }) => ({ Nombre: name, Clave: key, Email: email || '' })); const ws = lib.utils.json_to_sheet(data); const wb = lib.utils.book_new(); lib.utils.book_append_sheet(wb, ws, "Miembros"); lib.writeFile(wb, `miembros_${session.name.replace(/ /g, '_')}.xlsx`); };
            ...
            {!electionDetail && (
              <div className="md:hidden mt-4">
                <div role="tabpanel" id="panel-acreditacion" aria-labelledby="tab-acreditacion" className={`${activeTab === 'acreditacion' ? 'block' : 'hidden'} flex flex-col min-h-0`}>
                  ...
                  <div className="space-y-2 flex-1 min-h-[12rem] max-h-96 scroll-list pr-2 mb-4"> {filteredMembers.map(member => ( <div key={member.id} className="bg-slate-100 p-2 rounded-lg flex items-center justify-between gap-2"> <div className="min-w-0"><p className="font-semibold">{member.name}</p><p className={`text-sm ${member.status === 'Presente' ? 'text-green-600' : 'text-slate-500'}`}>{member.status === 'Invitado' ? 'Ausente' : member.status}</p></div> <div className="flex items-center gap-2 shrink-0"> {member.status === 'Invitado' ? <button onClick={() => session.id && member.id && onAccredit(session.id, member.id)} className="bg-green-500 hover:bg-green-600 text-white text-sm font-bold min-h-[44px] py-2 px-4 rounded-lg">Acreditar</button> : <div className="flex items-center gap-2"><span className="font-mono bg-slate-200 px-2 py-1 rounded text-sm">{member.key}</span><button type="button" onClick={() => session.id && member.id && onUnaccredit(session.id, member.id)} className="min-w-[44px] min-h-[44px] p-2 flex items-center justify-center rounded-lg text-slate-600 hover:bg-slate-100 hover:text-slate-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 focus-visible:ring-offset-2" title="Desacreditar" aria-label="Desacreditar"><DoorIcon /></button></div>} {user?.role === 'superadmin' && onDeleteMember && <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget({ type: 'member', id: member.id!, name: member.name }); }} className="min-w-[44px] min-h-[44px] p-2 flex items-center justify-center rounded-lg text-red-600 hover:bg-red-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 focus-visible:ring-offset-2" aria-label="Eliminar elector" title="Eliminar"><TrashIcon /></button>} </div> </div> ))} </div>
              ...
              <div role="tabpanel" className={`flex flex-col min-h-0 min-w-0 ${electionDetail || panelClosing ? 'overflow-hidden' : ''}`}>
                {(electionDetail || panelClosing) ? (
                  <>
                    ...
                    <div className="space-y-2 flex-1 min-h-0 overflow-y-auto scroll-list pr-1"> {filteredMembers.map(member => ( <div key={member.id} className="bg-slate-100 p-2 rounded-lg flex items-center justify-between gap-1 shrink-0"> <div className="min-w-0 flex-1"><p className="font-semibold text-sm truncate">{member.name}</p><p className={`text-xs ${member.status === 'Presente' ? 'text-green-600' : 'text-slate-500'}`}>{member.status === 'Invitado' ? 'Ausente' : member.status}</p></div> <div className="flex items-center gap-1 shrink-0"> {member.status === 'Invitado' ? <button onClick={() => session.id && member.id && onAccredit(session.id, member.id)} className="shrink-0 bg-green-500 hover:bg-green-600 text-white text-xs font-bold py-1 px-2 rounded">Acreditar</button> : <div className="flex items-center gap-1 shrink-0"><span className="font-mono bg-slate-200 px-1 py-0.5 rounded text-xs truncate max-w-[4rem]" title={member.key}>{member.key}</span><button type="button" onClick={() => session.id && member.id && onUnaccredit(session.id, member.id)} className="min-w-[36px] min-h-[36px] p-1 flex items-center justify-center rounded text-slate-600 hover:bg-slate-100" title="Desacreditar" aria-label="Desacreditar"><DoorIcon /></button></div>} {user?.role === 'superadmin' && onDeleteMember && <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget({ type: 'member', id: member.id!, name: member.name }); }} className="min-w-[36px] min-h-[36px] p-1 flex items-center justify-center rounded text-red-600 hover:bg-red-100" aria-label="Eliminar" title="Eliminar"><TrashIcon /></button>} </div> </div> ))} </div>
