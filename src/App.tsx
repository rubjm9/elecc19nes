...
function ResultsPage({ session, election, votes, onBack, onAddElection }: ResultsPageProps) {
    ...
    const electionVotes = Object.entries(votes).filter(([key]) => electionVoterKeys.has(key)).map(([, userVotes]: [string, { [electionId: string]: string[] }]) => userVotes[election.id]).filter(Boolean);
